...
df = pd.read_csv('Fundamentus.csv')


#print(df.to_string())  # Show All
cod = pd.Series(df["Código"])
#print(cod)
...
for i in range(0, len(cod)):
    ...
    Div2 = Div_Yield + 58
    Div3 = Div2 + 4
    new_df = pd.DataFrame({'Div. Yeld': [soup2[Div2:Div3]]}, index=[i])
    df.update(new_df)
    P_VP = soup2.find('P/VP')  # Div. Yield</span></td><td class="data"><span class="txt">
    P_VP2 = P_VP + 55
    P_VP3 = P_VP2 + 4
    neew_df = pd.DataFrame({'P/VP': [soup2[P_VP2:P_VP3]]}, index=[i])
    df.update(neew_df)
...
print(df)
    ...
    #df2 = pd.Series(df([["Div. Yeld"], [soup2[Div2:Div3]]]))
    #df.update(df2)

#print(df.to_string())  # Show All
#replace - https://acervolima.com/atualize-o-valor-da-coluna-de-csv-em-python/
